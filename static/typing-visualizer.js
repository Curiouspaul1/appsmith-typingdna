 ...
 *
 * Description
 * Adds a visual representation of the user's typing on selected inputs.
 *
 * Instructions & typical usage:
 * 1. add the class to the input on which you record the typing pattern.
 *
 * @param {object} options - class settings
 * @param {Boolean} options.showTDNALogo - display the TypingDNA logo.
 */

 export function TypingVisualizer(options) {
    if (!options) {
      options = {};
    }
    ...
    this.targets = {};
    this.showTDNALogo =
      options.showTDNALogo !== undefined ? options.showTDNALogo : true;
    this.logoHeight = 16;
  ...
  TypingVisualizer.prototype.removeTarget = function (target) {
    ...
    for (var i = 0; i < target.length; i++) {
      ...
      if (targetElement && this.targets.hasOwnProperty(targetElement.id)) {
        this.targets[targetElement.id].removeEventListener(
          "input",
          this.onChange
        );
      ...
      if (targetElement) {
        if (!this.targets.hasOwnProperty(targetElement.id)) {
          ...
          if (canvas && parent && containerDiv.appendChild(canvas.container)) {
            ...
            canvas.canvas.width =
              clientRect.width || this.typingLength * (self.deltaX + 2);
            canvas.canvas.height = Math.min(
              clientRect.height || this.vpMaxHeight,
              this.vpMaxHeight
            );
            ...
            this.setStyle(targetElement, {
              "padding-right":
                canvas.canvas.width +
                6 +
                (this.showTDNALogo ? this.logoHeight + 4 : 0) +
                "px",
            });
            var logo;
            ...
            this.targets[targetElement.id] = {
              ...
              updatePosition: function () {
                ...
                if (this.hasScroll !== hasScroll) {
                  ...
                  this.logo.style.right =
                    parseInt(this.logo.style.right) +
                    (hasScroll ? this.scrollOffset : -this.scrollOffset) +
                    "px";
                  this.canvasContainer.style.right =
                    parseInt(this.canvasContainer.style.right) +
                    (hasScroll ? this.scrollOffset : -this.scrollOffset) +
                    "px";
                }
              },
