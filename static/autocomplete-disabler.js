 ...
 */
 ...
 import { TypingVisualizer } from "./typing-visualizer.js";

 export function AutocompleteDisabler(options) {
 ...
 AutocompleteDisabler.prototype.getClonedElement = function (elem, id) {
   ...
   if (newElem.autocomplete && newElem.autocomplete.indexOf("new", 0) !== 0) {
   ...
   if (self.showTypingVisualizer) {
     var initTypingVisualizer = function () {
       if (!self.typingVisualizer) {
         self.typingVisualizer = new TypingVisualizer({
           showTDNALogo: self.showTDNALogo,
         });
       }
 ...
 AutocompleteDisabler.prototype.addTargetToTypingDNA = function (
   newNode,
   refNode
 ) {
   if (!newNode || !refNode) {
   ...
   for (var key in this.elements) {
     if (this.elements.hasOwnProperty(key)) {
       ...
       for (var i = 0; i < eventNames.length; i++) {
         ...
         this.elements[key].targetElement.addEventListener(
           eventName,
           eventHandler
         );
       }
